...
![](images/ch00_visual_summary.jpg)
...
**Researchers transitioning from Stata or R to Python** will appreciate the parallel structure that follows Cameron's familiar textbook while introducing Python's powerful ecosystem of data science libraries. Each chapter demonstrates how classic econometric techniques can be implemented using modern Python tools like Pandas and [PyFixest](https://pyfixest.org) (the Python port of R's fixest, used for all regressions with consistent syntax across Python, R, and Stata).
...
The cloud-based approach provides unprecedented accessibility. You can access your work from any device with an internet connection---start working on your desktop at home, continue on a laptop at a cafe, and review results on a tablet while commuting. There are no storage space requirements on your local machine; everything is saved in the cloud.
...
Every chapter includes a self-contained interactive dashboard that lets you explore key concepts visually---without writing any code. These single-file HTML applications use the same datasets as the chapter notebooks and feature sliders, toggles, and real-time Plotly.js charts that respond instantly to your input.

The dashboards are designed to build intuition before (or alongside) coding. For example, you can drag a sample-size slider and watch a sampling distribution narrow in real time, toggle between model specifications and see R² change instantly, or run a Monte Carlo simulation with one click and observe unbiasedness emerge. Each widget targets a specific Key Concept from the chapter, includes guided "Try this" prompts, and works offline after first load.

Interactive dashboards bridge the gap between passive reading and active coding. They let you experiment with econometric ideas at the speed of thought---no syntax errors, no runtime waits, just immediate visual feedback. Whether you use them as a preview before diving into the notebook, a review tool after completing a chapter, or a classroom demonstration aid, these dashboards add a hands-on dimension to every topic in the book.
...
> - Learning apps for every chapter (no coding required)
...
**Explore the interactive dashboard** for the chapter. Each chapter has a web app dashboard where you can manipulate sliders, toggles, and dropdowns to see key concepts in action---no coding required. Use it as a preview to build intuition before coding, or as a review tool to consolidate your understanding. The dashboards use the same datasets as the notebooks, so the numbers will match what you see in your code.
