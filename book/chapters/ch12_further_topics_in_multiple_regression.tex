...
Var($\hat{\beta}$)\_robust = (X'X)⁻¹ × [Σ eᵢ²xᵢxᵢ'] × (X'X)⁻¹ × n/(n-k)
...
Var($\hat{\beta}$) = \textbf{Bread} × \textbf{Meat} × \textbf{Bread}
...
\caption{Correlogram Showing Autocorrelation in GDP Growth Residuals}
\label{fig:ch12:correlogram}
\end{figure}
...
\item No autocorrelation: Cov(uᵢ, uⱼ) = 0 for i $\neq$ j
\end{itemize}
...
\item \textbf{Autocorrelation} (serial correlation): Cov(uₜ, uₜ₋ₛ) $\neq$ 0 for s > 0
...
1. \textbf{OLS remains unbiased}: E[$\hat{\beta}$] = $\beta$
2. \textbf{Standard errors are biased}: Usually too small (underestimate uncertainty)
3. \textbf{t-statistics inflated}: May falsely reject H_0
...
Var($\hat{\beta}$)\_HAC = (X'X)⁻¹ × [Σₜ eₜ²xₜxₜ' + Σₛ₌_1ᴸ wₛ Σₜ eₜeₜ₋ₛ(xₜxₜ₋ₛ' + xₜ₋ₛxₜ')] × (X'X)⁻¹
...
In this section, we construct prediction intervals for forecasting house prices at specific sizes. Prediction intervals differ fundamentally from confidence intervals—they account for both parameter uncertainty (sampling variability in β̂) and individual randomness (the error term u). While confidence intervals answer "What is the average price for all houses of this size?", prediction intervals answer "What will this particular house sell for?". We compute both manually and using statsmodels' \texttt{get\_prediction()} method to understand the mathematical foundations.

\begin{lstlisting}[language=Python]
# Simple regression: price on size
model_simple = ols('price ~ size', data=data_house).fit()

print("Simple regression: price = $\\beta_0$ + $\\beta_1$*size + u")
...
\caption{Confidence and Prediction Intervals for House Price Regression}
\label{fig:ch12:prediction-intervals}
\end{figure}
...
\textbf{1. Conditional Mean}: E[Y|X=x_0]
\begin{itemize}
...
\item \textbf{Uncertainty source}: Sampling variability in $\hat{\beta}_0$, $\hat{\beta}_1$
\end{itemize}

\textbf{2. Actual Value}: Y at X=x_0
\begin{itemize}
...
\item \textbf{Uncertainty sources}: (1) Sampling variability in $\hat{\beta}$ AND (2) individual random error u
\end{itemize}

\textbf{Why Prediction Intervals Are Wider}

\textbf{Conditional mean}: ŷ = β̂_0 + β̂_1x_0
\begin{itemize}
\item \textbf{Variance}: Var(ŷ) = Var(β̂_0) + x_0²Var(β̂_1) + 2x_0Cov(β̂_0,β̂_1)
\end{itemize}

\textbf{Actual value}: Y = β̂_0 + β̂_1x_0 + u
\begin{itemize}
...
SE(E[Y|X=x_0]) = σ̂ √[1/n + (x_0 - x̄)²/Σ(xᵢ - x̄)²]

\textbf{SE for actual value}:
SE(Y|X=x_0) = σ̂ √[1 + 1/n + (x_0 - x̄)²/Σ(xᵢ - x̄)²]
...
\textbf{Formula shows}: Intervals are \textbf{narrowest at x̄} (sample mean) and \textbf{wider} as x_0 moves away.

\textbf{For size}:
\begin{itemize}
\item \textbf{At x̄ = 1,883}: SE(ŷ) = σ̂/√n = \$23,551/√29 = \$4,374 (minimum)
\item \textbf{At x_0 = 2,000}: SE(ŷ) = \$4,565 (slightly wider, x_0 near x̄)
\item \textbf{At x_0 = 3,300} (max size): SE(ŷ) = \$9,214 (much wider, far from x̄)
\end{itemize}
...
\textbf{SE for conditional mean}:
SE(E[Y|X=x_0]) = √[x_0'(X'X)⁻¹x_0 × σ̂²]

\textbf{SE for actual value}:
SE(Y|X=x_0) = √[(1 + x_0'(X'X)⁻¹x_0) × σ̂²]
...
\item Compute $\hat{\beta}$ for each resample
...
\item Probability interpretation: "95\% probability $\beta$ is in this interval"
...
\textbf{Example}:
\begin{itemize}
\item \textbf{Prior}: $\beta$ ~ N(0, 100²) (vague prior)
\item \textbf{Likelihood}: y|X, $\beta$ ~ N(X$\beta$, σ²I)
\item \textbf{Posterior}: $\beta$|y, X ~ N($\hat{\beta}$, V) (combines prior and data)
\end{itemize}

\textbf{Advantages}:
\begin{itemize}
\item \textbf{Probability statements}: "95\% probability $\beta$ > 0" (not allowed in frequentist)
