...
\textbf{Context:} In this section, we establish the computational environment and prepare to analyze nine different datasets spanning education economics, production theory, macroeconomics, consumer behavior, and causal inference applications. Unlike previous chapters that focused on one or two datasets, Chapter 13 demonstrates the broad applicability of multiple regression by analyzing diverse economic problems. We set up output directories, configure plotting aesthetics, and prepare to load data from the open-source repository, ensuring all results are properly organized and reproducible.
...
2. \textbf{Causality requires identification}: Correlation ≠ causation; need RCT, DiD, RD, or IV for causal claims
...
\includegraphics[width=0.8\textwidth]{images/ch13_api_distribution.png}

\includegraphics[width=0.8\textwidth]{images/ch13_api_vs_edparent.png}
...
\includegraphics[width=0.8\textwidth]{images/ch13_api_correlation_matrix.png}
...
\textbf{1. Parent education (β̂ = 45.72)}:
\begin{itemize}
...
\textbf{2. Free/reduced meals (β̂ = -3.76)}:
\begin{itemize}
...
\textbf{3. English learners (β̂ = -0.79)}:
\begin{itemize}
...
\textbf{4. Year-round schools (β̂ = -28.47)}:
\begin{itemize}
...
\textbf{5. Credentialed teachers (β̂ = 0.96)}:
\begin{itemize}
...
\textbf{6. Emergency credential teachers (β̂ = 0.89, p = 0.126)}:
\begin{itemize}
...
\begin{itemize}
\item \textbf{H₀}: β_capital + β_labor = 1 (constant returns to scale)
\item \textbf{F-statistic}: 0.34
\item \textbf{p-value}: 0.564
\item \textbf{Decision}: Fail to reject H₀ at 5\% level
\item \textbf{Conclusion}: Data are consistent with constant returns to scale
\end{itemize}

\includegraphics[width=0.8\textwidth]{images/ch13_cobb_douglas_prediction.png}
...
\item β = labor elasticity of output
\end{itemize}
...
Where:
\begin{itemize}
\item β₀ = ln(A) (intercept estimates log productivity)
\item β₁ = α (capital elasticity)
\item β₂ = β (labor elasticity)
\end{itemize}
...
3. \textbf{Multiplicative errors}: Error enters multiplicatively (Q = A×K^α×L^β×exp(u)), which becomes additive in logs
4. \textbf{Linearization}: Makes nonlinear production function estimable by OLS

\textbf{Coefficient Interpretation}

\textbf{Capital elasticity (β̂_K = 0.232)}:
...
\textbf{Labor elasticity (β̂_L = 0.806)}:
...
\textbf{Total Factor Productivity (β̂₀ = -0.897)}:
...
\textbf{Formula}: α + β = sum of elasticities

\textbf{Interpretation}:
\begin{itemize}
\item \textbf{α + β = 1}: Constant returns to scale (CRTS)—doubling inputs → double output
\item \textbf{α + β > 1}: Increasing returns to scale (IRTS)—doubling inputs → more than double output (economies of scale)
\item \textbf{α + β < 1}: Decreasing returns to scale (DRTS)—doubling inputs → less than double output (diseconomies of scale)
\end{itemize}
...
\textbf{Hypothesis Test}: H₀: α + β = 1 vs. H₁: α + β ≠ 1

\textbf{Approach}: Compare unrestricted model to restricted model imposing CRTS.

\textbf{Restricted model} (imposing α + β = 1):
\begin{itemize}
\item Constraint: β = 1 - α
...
\item F = 0.34
\item p-value = 0.564
\item \textbf{Fail to reject} H₀: CRTS
\end{itemize}
...
2. \textbf{No autocorrelation}: Cov(u_t, u_s) = 0 for t ≠ s (errors independent across time)
...
The σ̂²/2 term corrects for \textbf{Jensen's inequality} (exp(E[X]) ≠ E[exp(X)] for random X). Without this, predictions would be systematically biased downward. Figure 13.4 includes this correction.
...
4. \textbf{Returns to scale}: Test α + β = 1 using restricted regression
...
\begin{itemize}
\item Add time trend: ln(Q) = β₀ + β₁×ln(K) + β₂×ln(L) + β₃×t
\end{itemize}
...
\includegraphics[width=0.8\textwidth]{images/ch13_phillips_pre1970.png}
...
\includegraphics[width=0.8\textwidth]{images/ch13_phillips_post1970.png}
...
\item β₃ (Expected inflation in full model): 0.829
\item β₂ (Unemployment in full model): -0.388
\item \textbf{Predicted bivariate coefficient}: -0.388 + 0.829 × 0.819 = \textbf{0.291}
\item \textbf{Actual bivariate coefficient}: 0.296
\item \textbf{✓ Match!} Omitted variables bias explains sign reversal.
\end{itemize}
...
\textbf{Equation}: π = β₀ + β₁×u + ε

Where:
\begin{itemize}
\item π = inflation rate
\item u = unemployment rate
\item β₁ < 0 expected (negative trade-off)
\end{itemize}

\textbf{Pre-1970 Evidence (Figure 13.5)}

\textbf{Coefficient (β̂₁ = -1.54)}:
...
\textbf{Unemployment coefficient (β̂₁ = -0.388)}:
...
\textbf{Expected inflation coefficient (β̂₂ = 0.829)}:
...
\textbf{Bias formula}: When true model is π = β₀ + β₁×u + β₂×π^e + ε, but we estimate π = b₀ + b₁×u + e (omitting π^e), then:
...
\textbf{Calculation}:
\begin{itemize}
\item β₁ = -0.388 (true unemployment effect)
\item β₂ = 0.829 (expected inflation effect)
...
\begin{itemize}
\item β₂ > 0 (expected inflation increases actual inflation)
\item γ > 0 (unemployment and expected inflation positively correlated post-1970)
\item \textbf{Bias} = β₂ × γ > 0 (positive)
\item True β₁ = -0.388 (negative)
...
\textbf{Advantages}:
1. \textbf{Unit-free elasticities}: β_j = \% change in mpg per 1\% change in X_j
...
4. \textbf{Log-linear in year}: β₄ = annual growth rate in efficiency (approximately)

\textbf{Coefficient Interpretations}

\textbf{Horsepower elasticity (β̂ = -0.285)}:
...
\textbf{Weight elasticity (β̂ = -0.619)}:
...
\textbf{Torque elasticity (β̂ = +0.213)}:
...
\begin{itemize}
\item Set weight-adjusted targets (account for β_weight = -0.62)
...
\begin{itemize}
\item \textbf{H₀}: All plan coefficients = 0 (insurance plan doesn't matter)
\item \textbf{F-statistic}: 13.91
\item \textbf{p-value}: < 0.0001
\item \textbf{Decision}: Reject H₀ at all conventional levels
...
\textbf{25\% coinsurance plan (β̂ = -\$117)}:
...
\textbf{50\% coinsurance plan (β̂ = -\$204)}:
...
\textbf{95\% coinsurance plan (β̂ = -\$232)}:
...
\textbf{Mixed and individual plans} (β̂ ≈ -\$137): Similar effects to 25\% plan.
...
1. \textbf{Association}: Cost-sharing and spending are correlated (✓ coefficients significant)
2. \textbf{Temporal precedence}: Insurance assignment came before spending (✓ experimental design)
3. \textbf{No confounding}: Randomization eliminates all confounders (✓ treatment/control identical on average)
...
\textbf{H₀}: β_25 = β_50 = β_95 = β_mixed = β_indiv = 0 (insurance doesn't matter)
...
\textbf{4. Low R² ≠ unimportant effects}: Heterogeneity can swamp treatment effects in R² but not in coefficients
...
\textbf{Coefficients}:
\begin{itemize}
\item \textbf{β₀}: Control group mean in pre-period
\item \textbf{β₁}: Treatment-control difference in pre-period (baseline difference)
\item \textbf{β₂}: Time trend in control group (secular change)
\item \textbf{β₃}: \textbf{DiD effect} (causal effect of clinic access)
\end{itemize}

\textbf{Results Interpretation}

\textbf{Intercept (β̂₀ = -1.24)}:
...
\textbf{Baseline difference (β̂₁ = -0.05, p=0.69)}:
...
\textbf{Time trend (β̂₂ = +0.12, p=0.018)}:
...
\textbf{DiD effect (β̂₃ = +0.14, p=0.035)}:
...
\textbf{DiD}: 0.26 - 0.12 = \textbf{0.14} ✓ (matches regression coefficient)
...
\textbf{Formula}: E[Y₁(0) - Y₀(0) | Treat] = E[Y₁(0) - Y₀(0) | Control]
...
1. \textbf{Pre-trends}: In our data, treatment and control had \textbf{similar baselines} (difference = -0.05, p=0.69). ✓

2. \textbf{Common shocks}: Both groups experienced same economic, political, environmental changes (post-apartheid South Africa). Likely ✓

3. \textbf{No compositional changes}: Same communities measured pre/post (no migration). ✓
...
\item \textbf{β₁} = \textbf{incumbency advantage} (causal effect of winning at threshold)
\end{itemize}
...
\textbf{Incumbency advantage (β̂₁ = 7.95\%)}:
...
\textbf{Assumption}: E[Y₀|margin=ε] and E[Y₁|margin=ε] are continuous at margin = 0.
...
\textbf{Naive OLS}: logGDP = β₀ + β₁×institutions + u
...
\begin{itemize}
\item Attenuation bias: E[β̂_OLS] → 0 as measurement error increases
\end{itemize}

\textbf{Consequence}: OLS coefficient β̂_OLS is \textbf{biased and inconsistent}. Cannot establish causality from correlation.

\textbf{Instrumental Variables Solution}

\textbf{Key idea}: Find an \textbf{instrument} Z that:
1. \textbf{Relevance}: Z affects institutions (Cov(Z, institutions) ≠ 0)
...
\textbf{Low R² doesn't mean IV is weak}: β̂_IV = 0.944 is precisely estimated (SE = 0.159, t = 5.94).

\textbf{IV Assumptions (Critical!)}

\textbf{1. Relevance} (testable): Instrument affects endogenous variable

\textbf{Test}: First-stage F-statistic = 27.34 >> 10 ✓
...
\textbf{Here}: Higher mortality → worse institutions for all countries (no heterogeneity in direction). Plausible ✓
...
\textbf{4. Heterogeneous treatment effects}: Allow β to vary by country characteristics (interactions).
...
2. \textbf{Cobb-Douglas production function and HAC standard errors}: US manufacturing (1899-1922) exhibited constant returns to scale (α + β = 1.04) with labor elasticity (0.806) dominating capital elasticity (0.232), using HAC standard errors to account for autocorrelation in time series data
...
6. \textbf{Bivariate Regression}: Correlation only—cannot establish causation due to omitted variables bias. Example: API vs. parent education (bivariate β = 134.8, multiple β = 45.7).
...
\begin{itemize}
\item \textbf{Chapters 6-7 (Bivariate Regression)}: Extended to multiple regression—Section 2 shows omitted variables bias (bivariate β = 134.8 vs. multiple β = 45.7 for parent education)
