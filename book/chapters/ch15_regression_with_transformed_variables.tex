...
In this section, we estimate quadratic models that capture the inverted U-shaped relationship between age and earnings—a fundamental pattern in labor economics known as the "age-earnings profile." Quadratic models add a squared term (age²) to the regression, allowing the marginal effect of age to vary by age itself. This specification reveals whether earnings increase early in careers, peak at middle age, then decline—a pattern that linear models cannot capture. We calculate marginal effects at different ages to show how the return to experience changes across the lifecycle.
...
\textbf{Statistical interpretation}: The linear model suggests each year of age increases earnings by \$525 (constant across all ages). But the quadratic model tells a richer story: the age coefficient is positive (+\$3,105) while the age-squared coefficient is negative (-\$29.66). This creates an inverted U-shape, with earnings increasing early in careers, reaching a maximum, then declining. The turning point occurs at age 52.34, calculated as -$\beta$_1/(2$\beta$_2) = -3105/(2×-29.66) = 52.34.
...
>
In quadratic models, the marginal effect varies by the level of X. For a model Y = $\beta$_0 + $\beta$_1X + $\beta$_2X², the marginal effect is ∂Y/∂X = $\beta$_1 + 2$\beta$_2X. This means the effect changes at every value of X. Three common ways to summarize: (1) \textbf{AME (Average Marginal Effect)}: average across all observations—most policy-relevant; (2) \textbf{MEM (Marginal Effect at Mean)}: effect at mean X—easiest to calculate; (3) \textbf{MER (Marginal Effect at Representative value)}: effect at specific X values of interest (e.g., age 25, 50, 65)—most interpretable for stakeholders.
\end{keyconcept}
...
In this section, we introduce interaction terms that allow the effect of one variable (education) to depend on the value of another (age). This tests whether the return to education varies across the lifecycle—do young workers benefit more or less from additional schooling than older workers? We create an interaction term by multiplying age × education, then include it alongside the main effects. Interpreting interactions requires calculating marginal effects at different values, revealing how relationships are heterogeneous across individuals rather than constant for everyone.
...
>
An interaction term (X_1 × X_2) allows the effect of X_1 to vary by X_2. For earnings = $\beta$_0 + $\beta$_1age + $\beta$_2education + $\beta$_3(age×education), the marginal effect of education is $\beta$_2 + $\beta$_3×age—different for each age. A positive $\beta$_3 means education's payoff increases with age; negative means it decreases. Always test interactions jointly with their main effects using F-tests, as multicollinearity often makes individual coefficients insignificant even when the variables matter collectively. Never interpret $\beta$_1 or $\beta$_2 alone when an interaction is present—they represent effects only when the other variable equals zero.
\end{keyconcept}
...
\textbf{Common pitfalls}: Students often forget that log model coefficients must be exponentiated for exact percentage effects: \%ΔY = (e^$\beta$ - 1) × 100. For small coefficients (|$\beta$| < 0.10), the approximation \%ΔY ≈ $\beta$ × 100 works well (0.1006 ≈ 10.06\%). But for larger coefficients, use the exact formula. Also, R² values are NOT comparable across levels and log specifications—they're explaining variance in different scales.

\begin{keyconcept}{Logarithmic Transformations and Percentage Effects}
>
Logarithmic transformations fundamentally change interpretation. In log-linear models (ln Y = $\beta$X), $\beta$ represents the proportional change in Y for a one-unit change in X: \%ΔY ≈ 100$\beta$ (exact: 100[e^$\beta$ - 1]). In log-log models (ln Y = $\beta$ ln X), $\beta$ is an elasticity: a 1\% change in X leads to a $\beta$\% change in Y. Log models are preferred for earnings because: (1) they handle skewed distributions better; (2) percentage effects are constant across income levels (fairness); (3) they prevent negative predictions; (4) they often fit better (higher R²). The tradeoff is interpretation complexity and retransformation bias when predicting levels.
\end{keyconcept}
...
\textbf{Statistical interpretation}: When we estimate ln(Y) = X$\beta$ + ε and predict Ŷ = exp(X$\beta$̂), the prediction is \textbf{biased downward}. The naive prediction averages \$45,838 (log-linear) vs. actual average of \$56,369—a massive 18.7\% underestimate! This occurs because the exponential function is convex: exp(E[ln(Y)]) $\neq$ E[Y]. Jensen's inequality tells us exp(E[ln(Y)]) < E[Y].
