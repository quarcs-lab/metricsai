...
\bottomrule
\end{tabular}
\caption{Population Regression of E[y|x] on x with Perfect Fit}
\label{tab:ch06:population-regression}
\end{table}
...
\caption{Population Regression Line E[y|x] = 1 + 2x Showing Perfect Deterministic Relationship}
\label{fig:ch06:population-line}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Sample Regression Results with Random Error Component}
\label{tab:ch06:sample-regression}
\end{table}
...
\caption{Sample Regression Line Showing Deviations from True Parameters Due to Sampling Error}
\label{fig:ch06:sample-regression-line}
\end{figure}
...
In this section, we generate three independent samples from the same data generating process to illustrate sampling variability. Each ``researcher'' collects their own data (n=30 observations) and estimates the same underlying relationship. This thought experiment demonstrates that different samples produce different estimates, yet all are valid realizations from the sampling distribution of the OLS estimator.
...
\caption{Sampling Distributions of OLS Estimators from 1,000 Monte Carlo Simulations Demonstrating Unbiasedness and Normality}
\label{fig:ch06:sampling-distributions}
\end{figure}
