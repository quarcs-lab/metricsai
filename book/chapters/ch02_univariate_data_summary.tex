...
In this section, we set up our Python environment and load the primary dataset---earnings data for 171 women aged 30. Proper data loading and initial inspection are critical first steps in any analysis because they allow us to understand the structure, data types, and completeness of our dataset before conducting any statistical analysis. We use pandas to stream data directly from a remote GitHub repository, demonstrating modern data science workflows that don't require local file storage.
...
\label{tab:ch02:basic-descriptive-stats}
\small
\begin{tabular}{lrrrr}
\toprule
...
\begin{table}[h]
\centering
\caption{Detailed Descriptive Statistics for Earnings Including Skewness and Kurtosis}
\label{tab:ch02:detailed-earnings-stats}
\begin{tabular}{lr}
\toprule
Statistic & Value \\
\midrule
...
\caption{Box Plot of Annual Earnings Distribution Showing Median, Quartiles, and Outliers (n=171)}
\label{fig:ch02:earnings-boxplot}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{../code_python/images/ch02_fig4_earnings_histograms.png}
\caption{Histogram of Annual Earnings Showing Right-Skewed Frequency Distribution (n=171)}
\label{fig:ch02:earnings-histogram}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{../code_python/images/ch02_fig5_earnings_kde.png}
\caption{Kernel Density Estimate of Earnings Distribution Showing Smooth Probability Density}
\label{fig:ch02:earnings-kde}
\end{figure}
...
In this section, we shift from numerical data (earnings) to categorical data (fishing mode choices), demonstrating that different data types require different analytical approaches. For categorical variables, we use frequency tables to count observations in each category and pie charts or bar charts to visualize the distribution. Understanding how to handle categorical data is essential because many economic variables---such as industry, occupation, region, or consumer choices---are inherently categorical rather than continuous.
...
\begin{table}[h]
\centering
\caption{Frequency Distribution of Fishing Modes (n=1,182)}
\label{tab:ch02:fishing-modes-frequency}
\begin{tabular}{lrr}
\toprule
...
\caption{Pie Chart of Fishing Mode Distribution Showing Charter and Private Boat Preference (n=1,182)}
\label{fig:ch02:fishing-modes-pie}
\end{figure}
...
In this section, we apply a logarithmic transformation to the skewed earnings data to make the distribution more symmetric and closer to normal. Log transformations are one of the most important tools in econometrics because many economic variables (income, GDP, prices, firm size) are naturally right-skewed with multiplicative relationships. Transforming such variables often improves statistical properties, makes relationships more linear, and facilitates interpretation in terms of percentage changes rather than absolute changes.
...
\begin{table}[h]
\centering
\caption{Comparison of Original and Log-Transformed Earnings Statistics}
\label{tab:ch02:earnings-log-comparison}
\begin{tabular}{lrr}
\toprule
...
\caption{Comparison of Original Earnings and Log-Transformed Earnings Distributions Showing Normalization Effect}
\label{fig:ch02:log-transformation}
\end{figure}
...
\begin{table}[h]
\centering
\caption{Summary Statistics for GDP and Related Variables (1959-2020, n=245)}
\label{tab:ch02:gdp-summary-stats}
\small
\begin{tabular}{lrrrr}
\toprule
...
\caption{U.S. Real GDP per Capita Time Series (1959-2020) Showing Long-Term Growth and Business Cycles}
\label{fig:ch02:gdp-timeseries}
\end{figure}
...
\textbf{Access the notebook here:} \url{https://colab.research.google.com/github/quarcs-lab/metricsai/blob/main/notebooks_colab/ch02_Univariate_Data_Summary.ipynb}
