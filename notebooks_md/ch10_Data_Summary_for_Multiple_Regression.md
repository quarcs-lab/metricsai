...
g = sns.pairplot(data_house[plot_vars], diag_kind='kde',
                 plot_kws={'alpha': 0.6, 's': 50})  # alpha = transparency, s = marker size
...
**What to look for in this actual vs. predicted plot:**

- **Alignment with 45-degree line**: Points close to the red dashed line indicate accurate predictions
- **Systematic deviations**: Clusters above or below the line suggest the model over- or under-predicts for certain price ranges
- **Spread**: Wider scatter around the line means larger prediction errors
...
- **Random scatter**: Residuals should appear randomly distributed around the zero line (no pattern)
- **Funnel shape**: A widening or narrowing pattern suggests heteroskedasticity (non-constant error variance)
- **Outliers**: Points far from the zero line may indicate unusual observations worth investigating
...
This single code block reproduces the core workflow of Chapter 10. It is self-contained — copy it into an empty notebook and run it to review the complete pipeline from data exploration and correlation analysis through multiple regression estimation, partial effects, model comparison, and multicollinearity diagnostics.
...
>
> - **Omitting relevant variables**: Leads to omitted variable bias in the remaining coefficients
> - **Including too many variables**: Can cause multicollinearity and inflate standard errors
> - **Ignoring high VIF values**: VIF > 10 suggests problematic multicollinearity
...
# DS4BOLIVIA DATASET — SATELLITE PREDICTORS
