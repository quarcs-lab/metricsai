...
**Key observation:** With more degrees of freedom (larger n), the t-distribution looks more like the normal. For n > 30, they are nearly identical.
...
t_crit_95 = stats.t.ppf(0.975, n - 1)  # 0.975 = upper tail for 95% two-sided CI
...
Our t-statistic falls inside the non-rejection region, so we do not reject H0. The data are consistent with mu = $40,000.
...
t_crit_upper = stats.t.ppf(0.95, n - 1)  # 0.95 for one-sided test
...
For an upper-tailed test, we only reject H0 if t is large and positive. Our t-statistic is below the critical value, so we do not reject.
...
z_crit = 1.96  # z-critical for 95% CI
...
> - **Confusing statistical significance with practical importance**: A tiny effect can be "significant" with a large sample
> - **Using a one-sided test when a two-sided test is appropriate**: One-sided tests require strong prior justification

**Python Libraries and Code:**

This single code block reproduces the core workflow of Chapter 4. It is self-contained — copy it into an empty notebook and run it to review the complete pipeline from sample statistics to confidence intervals, hypothesis tests, and inference for proportions.
...
(d) Test $H_0: \mu = 50$. What proportion of tests reject (should be $\approx 0.05$)?
...
# Two-sample t-test
print(f"Highest IMDS department: {top_dept}")
