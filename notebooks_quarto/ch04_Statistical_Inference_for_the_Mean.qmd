...
# Our t-statistic falls INSIDE the critical region,
# so we do NOT reject H0. The data are consistent with mu = $40,000.
```
...
# For an upper-tailed test, we only reject H0 if t is large and POSITIVE.
# Our t-statistic is below the critical value, so we do not reject.
```
