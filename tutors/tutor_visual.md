...
ax.plot(x_range, model.params['Intercept'] + model.params['sqft'] * x_range, 'r-', linewidth=2, label='OLS Line')
ax.set_xlabel('Square Footage')
ax.set_ylabel('Price ($1000s)')
ax.set_title(f'House Prices vs Size (R² = {model.rsquared:.2f})')
ax.legend()
...
```python
fig, ax = plt.subplots(figsize=(10, 6))
ax.scatter(model.fittedvalues, model.resid, s=50, alpha=0.7)
...
ci = model.conf_int().loc['sqft']
ax.barh('sqft', model.params['sqft'], xerr=[[model.params['sqft'] - ci[0]], [ci[1] - model.params['sqft']]], capsize=5, color='steelblue')
